...
mad_fixed_t III_requantize(unsigned int value, signed int exp)
{
  ...
  if (exp < 0) {
    if (-exp >= (int)sizeof(mad_fixed_t) * 8) {
      /* underflow */
      requantized = 0;
    }
    else {
...
{
  int i;
  mad_fixed_t const *bound = &xr[lines];
  for (xr += 18; xr < bound; xr += 18) {
    for (i = 0; i < 8; ++i) {
      register mad_fixed64hi_t hi;
      register mad_fixed64lo_t lo;
      register mad_fixed_t a = xr[-1 - i];
      register mad_fixed_t b = xr[     i];
...
{
  unsigned int i;
  register mad_fixed_t tmp1 = sample[1][sb];
  register mad_fixed_t tmp2 = sample[3][sb];
...
int mad_layer_III(struct mad_stream *stream, struct mad_frame *frame)
{
  ...
  if (header->flags & MAD_FLAG_PROTECTION) {
    header->crc_check =
      mad_bit_crc(stream->ptr, si_len * 8, header->crc_check);
  ...
  if (result == 0) {
    ...
    stream->anc_ptr    = ptr;
    stream->anc_bitlen = md_len * 8 - data_bitlen;
  }
